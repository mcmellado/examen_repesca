...
<body>
    ...
    $etiquetas = explode(" ", obtener_get('etiqueta'));
    $where = [];
    $execute = [];
    ...
    if (empty($etiquetas)) {
        ...
        foreach ($etiquetas as $key => $etiqueta) {
            $where[] = 'EXISTS (SELECT * FROM etiquetas_articulos WHERE etiquetas_articulos.id_articulo = articulos.id 
            AND etiquetas_articulos.id_etiqueta IN (SELECT id FROM etiquetas WHERE lower(nombre) LIKE lower(:etiqueta' . $key . ')))';
    ...
    <div class="container mx-auto">
        ...
        <form action="" method="get">
            ...
            <label>
                Etiqueta:
                <input type="text" name="etiqueta" value="<?= hh($etiqueta) ?>">
            </label>
                </p>
